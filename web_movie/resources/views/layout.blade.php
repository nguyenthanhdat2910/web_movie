...
<html lang="vi">
   <head>
      ...
      <link rel="shortcut icon" href="{{asset('upload/logo/'.$info -> logo)}}" type="image/x-icon" />
      <meta name="revisit-after" content="1 days" />
      <meta name='robots' content='index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1' />
      <title>Xem phim cùng HNP</title>
      ...
      <style>#header .site-title {background: url(https://www.pngkey.com/png/detail/360-3601772_your-logo-here-your-company-logo-here-png.png)
         no-repeat top left;background-size: contain;text-indent: -9999px;}</style>
   </head>
   <body class="home blog halimthemes halimmovies" data-masonry="">
            ...
            <div class="row" id="headwrap">
               <div class="col-md-3 col-sm-6 slogan">
                  <p class=""><a class="logo" href="" title="phim hay ">
                    <img height="100" width="100" src="{{asset('upload/logo/'.$info -> logo)}}">
                  </a></p>
               </div>
         ...
         <div class="container">
            <nav class="navbar halim-navbar main-navigation" role="navigation" data-dropdown-hover="1">
               <div class="navbar-header">
                  ...
                  <a href="" id="expand-ajax-filter" style="color: #ffed4d;">Lọc Phim<i class="fas fa"></i></a>
                  </button>
               </div>
               <div class="collapse navbar-collapse" id="halim">
                  <div class="menu-menu_1-container">
                     <ul id="menu-menu_1" class="nav navbar-nav navbar-left">
                        ...
                        @foreach($danhmuc_h as $key => $dm)
                           <li class="mega"><a title="{{$dm->title}}" href="{{route('danh_muc',$dm->slug)}}">{{$dm->title}}</a></li>
                        @endforeach
                        <li class="mega dropdown">
                           <a title="Thể Loại" href="#" data-toggle="dropdown" class="dropdown-toggle" aria-haspopup="true">Thể Loại <span class="caret"></span></a>
                           <ul role="menu" class=" dropdown-menu">
                              @foreach($theloai_h as $key => $thel)
                                 <li><a title="{{$thel->title}}" href="{{route('the_loai_phim',$thel->slug)}}">{{$thel->title}}</a></li>
                              @endforeach
                           </ul>
                        </li>
                        <li class="mega dropdown">
                           ...
                           <ul role="menu" class=" dropdown-menu">
                              @foreach($quocgia_h as $key => $quocg)
                                 <li><a title="{{$quocg->title}}" href="{{route('quoc_gia',$quocg->slug)}}">{{$quocg->title}}</a></li>
                              @endforeach
                           </ul>
                        </li>
                        <li class="mega dropdown">
            ...
            <div class="row container">
               <div class="widget about col-xs-12 col-sm-4 col-md-4">
                  <div class="footer-logo">
                     <img class="img-responsive" src="{{asset('upload/logo/'.$info -> logo)}}" alt="{{$info->title}}" />
                  </div>
      ...
      <div class="widget about col-xs-12 col-sm-4 col-md-12" ><p style="text-align: center; line-height:30px; color:blue"> @ XEM PHIM CÙNG HNP</p></div>
      <div id='easy-top'></div>
                  ...
                  $.getJSON('/json/phim.json', function(data){
                     $.each(data,function(key,value){
                        if(value.title.search(expression) != -1){
                           $('#result').append('<li class="list-group-item" style="cursor:pointer"><img height="40" width="40" src="upload/
                           phim/'+value.image+'">'+value.title+'</li>');
                        }
                     });
                  })
         ...
         $(document).ready(function(){
            ...
            $('.filter-sidebar').click(function(){
                  ...
                  $.ajax({
                     ...
                     success:function(data)
                        {
                           $('#halim-ajax-popular-post-home').css("display","none");
